//---------------------------------------------------------------------
//
//	File:	TStagePictureCue.cpp
//
//	Author:	Gene Z. Ragan
//
//	Date:	02.28.98
//
//	Desc:	Visual representation of a picture cue on the stage
//
...
TStagePictureCue::~TStagePictureCue()
{
...
status_t TStagePictureCue::Archive(BMessage *data, bool deep) const
{
	...
	{				
		...
		data->AddString("class", "TStagePictureCue");						
	}
...
void TStagePictureCue::Draw(BRect updateRect)
{	
...
void TStagePictureCue::DrawData(BRect updateRect, long theTime)
{	
...
void TStagePictureCue::MouseUp(BPoint where)
{
	// Do nothing if view is hidden
	if (IsHidden())
		return;
...
void TStagePictureCue::FrameResized(float new_width, float new_height)
{
...
void TStagePictureCue::SetVisibility(double theTime)
{
...
void TStagePictureCue::UpdatePictureCueRegion(double theTime)
{
	// Determine channelID
...
void TStagePictureCue::DrawSelectionRect()
{
	rgb_color saveColor;
	saveColor = HighColor();
...
void TStagePictureCue::InvalidateSelectionRect()
{
...
void TStagePictureCue::SetResizeZones()
{	
	BRect bounds = Bounds();
...
bool TStagePictureCue::PointInResizeZones(BPoint thePoint)
{
...
void TStagePictureCue::DragPicture(BPoint thePoint)
{
	BPoint 	savePt;
	uint32	buttons = 0;
...
void TStagePictureCue::ResizeOrCrop(BPoint thePoint)
{
...
short TStagePictureCue::GetResizeZoneID(BPoint thePoint)
{
	if (m_TopLeftResize.Contains(thePoint))
...
void TStagePictureCue::ResizeTopLeft( BPoint thePoint)
{
...
void TStagePictureCue::ResizeTopRight( BPoint thePoint)
{
...
void TStagePictureCue::ResizeRight( BPoint thePoint)
{
	// Resize the cue to the right will the mouse button is down	
	BPoint 	savePt;
	uint32	buttons = 0;
...
void TStagePictureCue::ResizeBottomRight( BPoint thePoint)
{
	// Resize the cue to the right will the mouse button is down	
	BPoint 	savePt;
	uint32	buttons = 0;
...
void TStagePictureCue::ResizeBottom( BPoint thePoint)
{
	// Resize the cue to the right will the mouse button is down	
	BPoint 	savePt;
	uint32	buttons = 0;
...
void TStagePictureCue::ResizeLeft( BPoint thePoint)
{
	// Resize the cue to the right will the mouse button is down	
	BPoint 	savePt;
	uint32	buttons = 0;
...
void TStagePictureCue::DoTransition(bool transitionIn)
{
